...
void EffectGenMore::generateEffectMore(Instruction* inst, BasicBlock* BB, Module* M)
{
...
//										errs() << "Effect op1: " << *(effect->getOperand(1)) << " op2: " << *(SI->getOperand(1)) << "\n";
							generateMetadata(unchangedString, currInst, zero, currInst, M);
							return;
						}								
					}
				}
...
void EffectGenMore::generateMetadata(MDString* str, Value* variable, Value* n, Instruction* inst, Module* M)
{
	LLVMContext& context = M->getContext();
	std::vector<Value*> effect;
